...
function divek_letrehozasa(x,y){
    ...
    let container = (ki_vagy_te === 'egyik')
        ? document.getElementById("egyik_palya")
        : document.getElementById("masik_palya");

    console.log("ki_vagy_te:", ki_vagy_te);
    console.log("container:", container);

    for (let i = 0; i < x; i++) {
        for (let j = 0; j < y; j++) {
            let div = document.createElement("div");
            div.id = `${ki_vagy_te}_${i}_${j}`;
            div.onclick = fovaros;
            container.appendChild(div);
        }
    }
}
function divek_szinezese(terkep){
    let prefix = ki_vagy_te;

    for (let y = 0; y < 24; y++) {
        for (let x = 0; x < 24; x++) {

            let div = document.getElementById(`${prefix}_${x}_${y}`);
            if (!div) {
                console.warn("Hiányzó div:", `${prefix}_${x}_${y}`);
                continue;
            }

            let biom = terkep[y][x][0];
...
function melyikez(div){
    let [, x, y] = div.id.split("_");
    return [parseInt(y), parseInt(x)];
}
function ezadiv(x,y){
    return document.getElementById(`${ki_vagy_te}_${x}_${y}`);
}
...
function dobaskereses(claimlist,dobas)
{
    let terkep = aktualis_terkep();
    let lehetsegesek = [];
    for (let y = 0; y < 23; y++) {
        for (let x = 0; x < 23; x++) {
            if(terkep[y][x][2] == dobas)
            {
                let koordinatak = [y,x];
                koordinatak.push(terkep[y][x][0]);
    ...
    else
    {
        ...
        for (let y = 0; y < 24; y++) {
            for (let x = 0; x < 24; x++) {
                ...
                let div = document.getElementById(`${ki_vagy_te}_${x}_${y}`);
                if (div) {
                    div.onclick = ut;
                }
            }
...
function expedicio(e)
{
    ...
    if(nyersanyaglista[1] >= 5,nyersanyaglista[0] >= 2,nyersanyaglista[3] >= 2)//5fa 2széna 2szövet
    {
        ...
        else
        {
            ...
            nyersanyaglista[0] -= 2;
            nyersanyaglista[3] -= 2;
            let d1 = lelohelye(x-1,y-1,terkep)
            if(d1 == 1)
            {
                e.target.innerHTML='M';
                console.log("teli találat");
            }
            let d2 = lelohelye(x-1,y-1,terkep)
            if(d2 == 1)
            {
                e.target.innerHTML='M';
                console.log("teli találat");
            }
            let d3 = lelohelye(x-1,y-1,terkep)
            if(d3 == 1)
            {
                e.target.innerHTML='M';
                console.log("teli találat");
            }
            let d4 = lelohelye(x-1,y-1,terkep)
            if(d4 == 1)
            {
                e.target.innerHTML='M';
                console.log("teli találat");
            }
            let d5 = lelohelye(x-1,y-1,terkep)
            if(d5 == 1)
            {
                e.target.innerHTML='M';
                console.log("teli találat");
            }
            let d6 = lelohelye(x-1,y-1,terkep)
            if(d6 == 1)
            {
                e.target.innerHTML='M';
                console.log("teli találat");
            }
            let d7 = lelohelye(x-1,y-1,terkep)
            if(d7 == 1)
            {
                e.target.innerHTML='M';
                console.log("teli találat");
            }
            let d8 = lelohelye(x-1,y-1,terkep)
            if(d8 == 1)
            {
                e.target.innerHTML='M';
                console.log("teli találat");
            }
            let d9 = lelohelye(x-1,y-1,terkep)
            if(d9 == 1)
            {
                e.target.innerHTML='M';
                console.log("teli találat");
...
function kapcsolodike(x,y)
{
    let terkep = aktualis_terkep();
    let valasz = 0;
    if(terkep[y-1][x-1][3] == 1)//1
    {
        valasz = 1;
    }
    else if(terkep[y-1][x][3] == 1)//2
    {
        valasz = 1;
    }
    else if(terkep[y-1][x+1][3] == 1)//3
    {
        valasz = 1;
    }
    else if(terkep[y][x-1][3] == 1)//4
    {
        valasz = 1;
    }
    
    else if(terkep[y][x+1][3] == 1)//6
    {
        valasz = 1;
    }
    else if(terkep[y+1][x-1][3] == 1)//7
    {
        valasz = 1;
    }
    else if(terkep[y+1][x][3] == 1)//8
    {
        valasz = 1;
    }
    else if(terkep[y+1][x+1][3] == 1)//9
    {
        valasz = 1;
    }
    return valasz;
...
let ki_vagy_te;
let username;
terkep_egyik = randommapgen();
terkep_masik = randommapgen();
async function init()
{
    ...
    console.log("Játékos betöltve:", ki_vagy_te, username);
...
async function interfesz_generalasa()
{
    ...
    divek_szinezese(aktualis_terkep());
    if (ki_vagy_te == 'masik') {
        divek_letrehozasa(24, 24);   
        divek_szinezese(aktualis_terkep());
    }
...
button.addEventListener("click", function () {
    ...
    if (ki_vagy_te == 'masik') {
        nyersanyagosztas(dobott,masik_palya)
    }
    else{
        nyersanyagosztas(dobott,egyik_palya)
    }
    alert(dobott);
});
